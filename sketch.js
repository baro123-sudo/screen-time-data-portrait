...
function drawGirl(cx, cy, s) {
  let skin = color('#F5C9A0');
  let hair = color('#3D1F00');
  ...
  bezierVertex(cx-s*0.7,cy+s*0.1, cx-s*0.75,cy+s*0.6, cx-s*0.55,cy+s*1.1);
  vertex(cx - s*0.38, cy + s*1.1);
  ...
  bezierVertex(cx-s*0.65,cy+s*0.75, cx-s*0.72,cy+s*1.0, cx-s*0.68,cy+s*1.35);
  vertex(cx + s*0.68, cy + s*1.35);
  ...
  let p = 0.7 + 0.18 * sin(frameCount * 0.06);
  c.setAlpha(p * 255);
  fill(c);
  let si = s*0.022;
